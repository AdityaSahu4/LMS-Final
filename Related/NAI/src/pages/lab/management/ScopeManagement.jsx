...
import { EquipmentMaintenancePredictor } from '../../../components/aiFeatures'
      ...
      case 'equipment':
        return (
          <div className="space-y-8">
            ...
            {/* AI Equipment Maintenance Predictions */}
            {equipments.length > 0 && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">AI Maintenance Predictions</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {equipments.slice(0, 2).map((equipment) => (
                    <EquipmentMaintenancePredictor
                      key={equipment.id}
                      equipment={{
                        ...equipment,
                        last_calibration_date: equipment.calibrationDate,
                        validity_date: equipment.validityDate,
                        purchase_date: equipment.purchaseDate || new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString()
                      }}
                      history={[]}
                      onScheduleMaintenance={(type, deadline) => {
                        toast.success(`Scheduling ${type} for ${equipment.name}`)
                      }}
                    />
                  ))}
                </div>
